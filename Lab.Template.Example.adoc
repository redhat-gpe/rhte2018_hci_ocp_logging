...
:cluster1_base: rhte-cloud1.example.opentlc.com
:cluster2_base: rhte-cloud2.example.opentlc.com
...
Make sure you can log in to both OpenShift clusters.

[NOTE]
You will be using two separate clusters in this lab. The cluster URLs are https://{cluster1_master} and https://{cluster2_master}.

. Use the `oc login` command to log in to both clusters, making sure to replace the URLs with your cluster's URLs:
+
[source,text]
----
...
* Use your *OpenTLC* short user name (e.g. `wkulhane` if your OpenTLC long user name is `wkulhane-redhat.com`) as the Username.
* Use your *Red Hat* e-mail address (e.g. `wkulhane@redhat.com`) for the E-mail address
* Use any password
...
. Your private repository is now `{quay_hostname}/<Your OpenTLC Short Name>/rhte-app`.
...
. In the top right corner Click on your account name and the *Account Settings*
. On the left hand side click the little robot icon (the second icon from the top)
. On the *Robot Accounts* page click the button on the top right corner *+ Create Robot Account*
. In the *Create Robot Account* panel use `jenkins` as the name for the robot account. Feel free to add a description as well. Then click *Create Robot Account*. The full name of the Robot Account will be the combination of your username and the robot account name - e.g. `wkulhane+jenkins`.
...
. Now click on the *Robot Account Name* (it should be your shortname followed by `+jenkins`, e.g. `wkulhane+jenkins`).
...
. Log out of Quay (or simply close the browser window).
...
. On your Laptop set up an environment variable with your project prefix (in the example below `<shortname>`). *Make sure to replace `<shortname>` with your shortname* which is the same you used when you registered for Quay - e.g. `wkulhane` in the examples above).
+
[source,bash]
----
export GUID=<shortname>
echo "export GUID=$GUID" >> ~/.bashrc
echo $GUID
----
+
. Log into the OpenShift Cluster 1 (use https://{cluster1_master} as `<CLUSTER_URL>`).
+
[source,bash]
----
...
+
[NOTE]
Because you are using a shared cluster all project names need to be unique. Using your OpenTLC shortname should guarantee that your project names are unique to the cluster.
+
[source,bash]
----
...
----
+
. Create your Jenkins Instance and assign sufficient resources for this instance.
+
[source,bash]
----
oc new-app jenkins-persistent --param ENABLE_OAUTH=true --param MEMORY_LIMIT=2Gi --param VOLUME_CAPACITY=1Gi -n ${GUID}-jenkins
...
def cluster1_base = "rhte-cloud1.example.opentlc.com"
...
+
====
http://rhte-app-xyz123-rhte-app.apps.{cluster1_base}
====
+
